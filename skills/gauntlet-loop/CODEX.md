...
5. Fan out workers for **product** areas (parallel where independent; single owner on coupled seams).
6. Grab the cheapest glanceable frames; run a **separate** critic pass with no builder context.
...
**After every critic round:** update `GAUNTLET_STATE.md`, then keep tool-calling into round N+1 **on the product** in the same turn. "Reference won the blind A/B" means continue, not stop.

Second failure mode: optimizing capture/eval tooling instead of the artifact. Capture is disposable (AGENTS.md §3). Pull workers back to the product.
...
- No capture/eval "product" inside the project

## Fan-out

- Independent **product** subsystems → parallel workers
